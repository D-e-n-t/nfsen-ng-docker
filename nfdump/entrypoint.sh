...
cat /tmp/sources.set | while read ln; do
    ...
    mkdir -p /data/live/$host && command="${protocol}fcapd -I $host -l /data/live/$host  -w -S 1 -T all -p $port -e"
    if [ -z "$command" ]; then 
...
while true; do
    ...
    cat /tmp/sources.set | while read ln; do
        read -r host port protocol <<< $(echo $ln | awk -F ';' '{print $1 " " $2 " " substr($3,1,1)}')
        command="${protocol}fcapd -I $host -l /data/live/$host  -w -S 1 -T all -p $port -e"
